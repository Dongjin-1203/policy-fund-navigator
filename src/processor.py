...
class  GovernmentNoticeLoader(BaseLoader):
    ...
    def _extract_pdf_with_coords(self):
        ...
        좌표 기반으로 표 영역을 감지하여 텍스트 중복을 방지하고,
        표는 마크다운으로 따로 추출
        """
        all_text = ""
        # pdfplumber로 PDF 열기
        with pdfplumber.open(self.file_path) as pdf:
            ...
            for page in pdf.pages:
                ...
                # 표가 차지하는 영역(bbox) 가져오기
                table_bboxes = [t.bbox for t in tables]

                # 글자의 좌표가 표 범위(bbox) 안에 있으면 제외하는 함수
                def not_within_table(obj):
                    # obj가 글자(char)인 경우, 그 좌표가 어떤 표의 범위 안에도 들어있지 않아야 함
                    if obj.get("object_type") == "char":
                        for bbox in table_bboxes:
                            x0, top, x1, bottom = bbox
                            # 글자의 중심점이 표 사각형 안에 있는지 체크
                            if (x0 <= obj["x0"] <= x1) and (top <= obj["top"] <= bottom):
                                return False
                    return True
                
                # 표 영역을 도려낸(filter) 페이지 객체에서 텍스트 추출
    ...
    def _extract_hwp_with_labels(self):
        """ HWP에서 텍스트 추출하는 내부 함수
        HWP 바이너리 구조에서 BodyText 영역을 찾아
        zlib로 압축된 텍스트를 추출
        """
        f = olefile.OleFileIO(self.file_path)
        dirs = f.listdir()

        # 실제 텍스트가 담긴 BodyText 영역 찾기
        bodytext_dirs = [d for d in dirs if 'BodyText' in d]
        text_content = ""

        for section in bodytext_dirs:
            section_data = f.openstream(section).read()
            # zlib로 압축된 데이터를 풀어서 텍스트 스트림 확보
            decompressed_data = zlib.decompress(section_data, -15)  # -15: zlib 헤더 없이 압축된 데이터 처리

            # 바이너리에서 문자열 추출 (간단한 디코딩 방식)
            i = 0
            size = len(decompressed_data)
            while i < size:
                header = struct.unpack('<I', decompressed_data[i:i+4])[0]  # <I: little-endian unsigned int
                rec_type = header & 0x3ff  # 하위 10비트
                rec_len = (header >> 20) & 0xfff   # 상위 12비트

                if rec_type == 67:  # 67: 텍스트 레코드 타입
                    data = decompressed_data[i+4:i+4+rec_len]
                    text_content += data.decode('utf-16', errors='ignore') + " "
                elif rec_type == 11: # 11: 표 시작을 알리는 컨트롤 레코드
                    text_content += "\n\n[Table Start]\n"
                elif rec_type == 66: # 66: 문단 헤더 (보통 표 내부나 문단이 바뀔 때 발생)
                    text_content += "\n[Row/Cell Boundary]\n"
                # 표나 구역이 바뀔 때 구분자 삽입
                elif rec_type == 68:  # 68: 구역이 바뀌는 지점
                    text_content += "\n[Section Boundary]\n"

                i += 4 + rec_len  # 다음 레코드로 이동
        return text_content
...
def create_analysis_chain():
    ...
    4. 'sub_title': 각 세부 사업의 명칭을 기제하세요. 별도 세부명이 없다면 'main_title'과 동일하게 표기하세요.
...
if __name__ == "__main__":
    ...
    file_list = glob.glob(os.path.join(folder_path, "*.pdf")) + glob.glob(os.path.join(folder_path, "*.hwp"))
    ...
    for file_path in file_list:
        try:
            print(f"--- 현재 처리 중: {os.path.basename(file_path)} ---")
            # 데이터 로딩
            ...
            print(f"완료: {result['source_file']}")
        
        except Exception as e:
            print(f"!!! {file_path} 처리 중 오류 발생: {e} !!!")

# 최종 결과 출력 (혹은 JSON 파일로 저장)
with open("analysis_results.json", "w", encoding="utf-8") as f:
    json.dump(final_results, f, ensure_ascii=False, indent=4)

print("--- 모든 파일 분석이 완료되었습니다. 결과가 analysis_results.json에 저장되었습니다. ---")